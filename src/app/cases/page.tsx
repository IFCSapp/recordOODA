...
import { EmptyState, Input, Label, Notice, PageHeader, Section, SubmitButton, Tag } from "@/components/ui";
...
export default async function CasesPage({ searchParams }: { searchParams?: { error?: string } }) {
          ...
          <form action={createCaseAction} className="mt-4 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
            {errorMessage ? (
              <div className="md:col-span-3">
                <Notice tone="warn">{errorMessage}</Notice>
              </div>
            ) : null}
            <Label>
              表示名
