...
export default async function OrientPage({ searchParams }: { searchParams?: { observationId?: string } }) {
      ...
      <Section title="見立ての立体マップ" description="入力中の仮説が、上のマップに反映されます。考え方の位置関係を見ながら整理できます。">
        {observations.length === 0 ? (
          ...
          <form action={createHypothesesAction} className="grid gap-5">
            ...
            <SubmitButton>仮説を保存してDecideへ</SubmitButton>
          </form>
        )}
      </Section>
  ...
  return (
    <fieldset className={`rounded-md border border-ink/10 bg-white p-4 shadow-sm hypothesis-input-card hypothesis-input-card-${index + 1}`}>
      ...
      <div className="grid gap-3">
        ...
        <Label>
          仮説文
          <Textarea name={`statement-${index}`} rows={3} data-hypothesis-statement="true" placeholder="〜が影響している可能性がある" />
        </Label>
        <Label>
          根拠となる観察
          <Textarea name={`evidence-${index}`} rows={3} placeholder="どの事実からそう考えたか" />
        </Label>
        <Label>
          反証または別解釈
