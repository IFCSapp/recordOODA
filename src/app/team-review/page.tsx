...
export default async function TeamReviewPage({ searchParams }: { searchParams?: { caseId?: string; targetRef?: string; columnKey?: string; error?: string } }) {
  const cases = await prisma.case.findMany({ orderBy: { updatedAt: "desc" } });
  const selectedCaseId = searchParams?.caseId ?? cases[0]?.id;
  const selectedTargetRef = searchParams?.targetRef ?? "";
  const selectedColumnKey = searchParams?.columnKey ?? "次の確認点";
  ...
  const observations = selectedCaseId
    ...
    : [];
  const caseComments = selectedCaseId
    ? await prisma.teamReviewComment.findMany({
        where: {
  ...
  const reviewComments = observations
    .flatMap((observation) => [
      ...observation.teamReviewComments.map((comment) => ({
        comment,
        context: `${formatDateTime(observation.observedAt)} / ${observation.programName}`
      })),
      ...observation.hypotheses.flatMap((hypothesis) =>
        hypothesis.teamReviewComments.map((comment) => ({
          comment,
          context: `${formatDateTime(observation.observedAt)} / ${observation.programName} / ${hypothesis.category}`
        }))
      )
    ])
    .concat(caseComments.map((comment) => ({ comment, context: "ケース全体" })))
    ...
    <>
          ...
          <div className="overflow-x-auto rounded-md border border-ink/10 bg-white shadow-sm">
            <table className="min-w-[1180px] border-collapse text-left text-sm">
              <thead className="bg-field text-xs text-ink/65">
                <tr>
                  {["事実", "仮説", "根拠", "反証・未確認点", "試した支援", "反応", "次の確認点", "コメント"].map((label) => (
              ...
              <tbody>
                {rows.map(({ observation, hypothesis }) => {
                  ...
                  const targetRef = `${observation.id}|${hypothesis?.id ?? ""}`;
                  const commentHref = `/team-review?caseId=${selectedCaseId}&targetRef=${encodeURIComponent(targetRef)}&columnKey=${encodeURIComponent("次の確認点")}#review-comment-form`;
                  return (
                    <tr key={`${observation.id}-${hypothesis?.id ?? "none"}`} className="align-top">
                      ...
                      <Cell>
                        <Link href={commentHref} className="focus-ring rounded-md border border-ink/15 bg-white px-3 py-2 text-xs font-semibold text-ink transition hover:border-skyline hover:text-skyline">
                          この行にコメント
                        </Link>
                      </Cell>
                    </tr>
      ...
      {selectedCaseId ? (
        <Section title="レビューコメント">
          <form id="review-comment-form" action={createTeamReviewCommentAction} className="grid gap-4 rounded-md border border-ink/10 bg-white p-4 shadow-sm md:grid-cols-[1fr_1fr]">
            {errorMessage ? (
              <div className="md:col-span-2">
                <Tag>保存できませんでした</Tag>
                <div className="mt-2 rounded-md border border-clay/35 bg-clay/10 px-4 py-3 text-sm leading-6 text-clay">{errorMessage}</div>
              </div>
            ) : null}
            <input type="hidden" name="caseId" value={selectedCaseId} />
            <Label>
              対象
              <Select name="targetRef" defaultValue={selectedTargetRef}>
                <option value="">ケース全体</option>
                {rows.map(({ observation, hypothesis }) => (
                  <option key={`${observation.id}-${hypothesis?.id ?? "none"}`} value={`${observation.id}|${hypothesis?.id ?? ""}`}>
                    {formatDateTime(observation.observedAt)} / {observation.programName} / {hypothesis?.category ?? "観察のみ"}
                  </option>
                ))}
              </Select>
            </Label>
            <Label>
              列
              <Select name="columnKey" defaultValue={selectedColumnKey}>
          ...
          <div className="mt-4 grid gap-3">
            {reviewComments.length === 0 ? (
              ...
              reviewComments.map(({ comment, context }) => (
