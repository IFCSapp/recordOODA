...
export default async function ObservePage({ searchParams }: { searchParams?: { caseId?: string; error?: string } }) {
      ...
      <Section title="観察を入力" description="60〜90秒で残せるよう、短い入力とチェックを中心にしています。">
        {cases.length === 0 ? (
          ...
          <form action={createObservationAction} className="grid gap-5 rounded-md border border-ink/10 bg-white p-4 shadow-sm">
            ...
            <details className="rounded-md border border-ink/10 bg-field/60 p-4">
              <summary className="cursor-pointer text-sm font-semibold text-ink">詳しく残す（数値・環境・身体心理）</summary>
            ...
            <Notice tone="warn">
              緊急度が高い、自傷他害や医療確認が必要な場合は、この記録フローではなく事業所の緊急対応フローを優先してください。
            </Notice>
