...
export default async function DecidePage({ searchParams }: { searchParams?: { hypothesisId?: string; observationId?: string; error?: string } }) {
        ...
        {hypotheses.length === 0 ? (
          ...
          <form action={createSmallExperimentAction} className="grid gap-5 rounded-md border border-ink/10 bg-white p-4 shadow-sm">
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              ...
              <Label>
                次回候補1
                <Textarea name="nextTryCandidate" rows={5} placeholder="同時に実施せず、次に試す候補として保存" />
              </Label>
              <Label>
                次回候補2
                <Textarea name="nextTryCandidate" rows={5} placeholder="必要なら別候補として保存" />
              </Label>
            </div>
            <Notice>次回候補は保存されます。今回は「試す支援」だけを実施します。</Notice>
            ...
            <SubmitButton>支援を一つ保存して、反応記録へ</SubmitButton>
          </form>
        )}
      </Section>
            ...
            {hypotheses.flatMap((hypothesis) =>
              hypothesis.smallExperiments.map((experiment) => (
                <Link key={experiment.id} href={`/act?experimentId=${experiment.id}`} className="rounded-md border border-ink/10 bg-white p-4 shadow-sm transition hover:border-skyline">
                  ...
                  {experiment.nextTryCandidate ? (
                    <p className="mt-2 text-xs leading-5 text-ink/55">次回候補: {experiment.nextTryCandidate}</p>
                  ) : null}
