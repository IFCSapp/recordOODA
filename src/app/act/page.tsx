...
export default async function ActPage({ searchParams }: { searchParams?: { experimentId?: string; error?: string } }) {
          ...
          <form action={createActReviewAction} className="grid gap-5 rounded-md border border-ink/10 bg-white p-4 shadow-sm">
            ...
            {selected ? (
              <div className="rounded-md border border-skyline/20 bg-skyline/10 p-4 text-sm leading-6 text-ink/75">
                ...
                {selected.nextTryCandidate ? <p className="mt-1 text-ink/55">次回候補: {selected.nextTryCandidate}</p> : null}
              </div>
            ) : null}
            ...
            <Label>
              次に試す候補
              <Textarea name="nextTryCandidate" rows={3} defaultValue={selected?.nextTryCandidate ?? ""} placeholder="同時に実施せず、次の小さな試行候補として残す" />
            </Label>
            ...
            <SubmitButton>反応を保存して、チーム確認へ</SubmitButton>
          </form>
        )}
      </Section>
