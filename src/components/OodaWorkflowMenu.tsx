    ...
    <nav aria-label="OODAの流れ" className="workflow-menu-shell">
      ...
      >
        ...
        <div className="ooda-orbit-track">
          ...
          <div className="ooda-step-link-row" onPointerDown={(event) => event.stopPropagation()}>
            {oodaNavItems.map((item) => (
              <Link key={item.href} href={item.href} className={item.tone} aria-current={pathname.startsWith(item.href) ? "page" : undefined}>
